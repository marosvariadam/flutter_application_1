...
class _WorkoutBuilderPageState extends State<WorkoutBuilderPage> {
    ...
    setState(() {
      _exercises.add(
        _BuilderExercise(
          ...
          sets: [_BuilderSet(weight: null, reps: null)],
        ),
      );
    });
  ...
  void _addSet(int exerciseIndex) {
    setState(() {
      _exercises[exerciseIndex].sets.add(_BuilderSet(weight: null, reps: null));
    });
  }
  ...
  void _showExercisePicker() {
    showModalBottomSheet(
      ...
      builder: (_) => _ExercisePicker(onSelect: _addExercise),
    );
  }
  ...
  void _save() {
    if (_titleController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Adj meg egy edzés nevet!')),
      );
      return;
    }
    if (_selectedAthleteId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Válassz atléta!')),
      );
      return;
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Edzés elmentve: ${_titleController.text}'),
        ...
        children: [
          ...
          ..._exercises.asMap().entries.map(
            (entry) => _ExerciseBuilderCard(
              ...
              onWeightChanged: (setIdx, val) =>
                  setState(() => _exercises[entry.key].sets[setIdx].weight = val),
              onRepsChanged: (setIdx, val) =>
                  setState(() => _exercises[entry.key].sets[setIdx].reps = val),
            ),
          ),
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(DT.rCardSmall),
                border: Border.all(
                  color: isSelected ? opt.$2 : DT.borderGrey,
                  width: 2,
                ),
              ),
              child: Text(
...
class _ExerciseBuilderCard extends StatelessWidget {
  ...
  final void Function(int setIdx, double?) onWeightChanged;
  final void Function(int setIdx, int?) onRepsChanged;
        ...
        children: [
          ...
          Expanded(
            child: _NumInput(
              ...
              hint: '0',
              onChanged: (v) => onRepsChanged(int.tryParse(v)),
              isDecimal: false,
            ),
          ),
          SizedBox(
            ...
            child: canRemove
                ? GestureDetector(
                    onTap: onRemove,
                    child: const Icon(Icons.remove_circle_outline, size: 18, color: DT.cardRed),
                  )
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          border: InputBorder.none,
          contentPadding: const EdgeInsets.symmetric(vertical: DT.s2),
        ),
        onChanged: onChanged,
      ),
    );
...
class _ExercisePickerState extends State<_ExercisePicker> {
  ...
  @override
  Widget build(BuildContext context) {
    final library = WorkoutMockData.exerciseLibrary;
          ...
          child: Row(
            children: [
              ...
              GestureDetector(
                ...
                child: const Icon(Icons.close, color: DT.iconLight),
              ),
            ],
          ),
        ),
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                title: Text(
                  ex['name']!,
                  style: const TextStyle(fontWeight: FontWeight.w500, color: DT.textPrimary),
                ),
                subtitle: Text(
                  ex['category']!,
